...
namespace Gears.Interpreter.Core.Data.Serialization.Mapping
{
    ...
    public class DictionaryToProxyMapper : IDictionaryToObjectMapper
    {
        private readonly ILazyExpressionResolver _lazyExpressionResolver;

        public DictionaryToProxyMapper(ILazyExpressionResolver lazyExpressionResolver)
        {
        ...
        {
            var contructors = type.GetConstructors().OrderBy(c => c.GetParameters().Count());
